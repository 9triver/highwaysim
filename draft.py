...
class DataParser:
    ...
    @classmethod
    def calculate_prob(cls) -> None:
        df = pd.read_csv(
            cls.RESOURCE_PATH + rf"{cls.province}/statisticalData/hourly_entry_count.csv", dtype=str
        )
        total_num = 0
        hex_2_count = {}
        for row in [x[1] for x in df.iterrows()]:
            ...
            if h in cls.valid_entrance_hex_list:
                num:int = int(row.iloc[2])
                total_num += num
                hex_2_count[h] = num + hex_2_count.get(h, 0)
        for k, v in hex_2_count.items():
            cls.entry_prob.append((cls.hex_2_entrance[k], float(v)/total_num))
